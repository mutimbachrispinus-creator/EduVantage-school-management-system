...
export function getProfessionalRemarks(pct, curriculum = 'CBC', tier = 'teacher') {
  if (curriculum === 'CBC') {
    if (pct >= 80) {
      return tier === 'teacher' ? 'An exceptional performance! You have shown mastery of the concepts. Keep it up.' : 'Outstanding result. You are a role model for academic excellence.';
    } else if (pct >= 60) {
      return tier === 'teacher' ? 'Good work! You meet the expectations in most areas. Aim higher next term.' : 'A commendable performance. Keep working hard to reach the top level.';
    } else if (pct >= 40) {
      return tier === 'teacher' ? 'Steady progress. You are approaching the required standards; more focus is needed in weaker areas.' : 'Fair performance. With more dedication, you can achieve better results.';
    } else {
      return tier === 'teacher' ? 'Performance is below expectations. You need to put in more effort and seek guidance in challenging subjects.' : 'A disappointing result. Urgent intervention and parent consultation are required.';
    }
  } else if (curriculum === 'CAMBRIDGE' || curriculum === 'BRITISH') {
    if (pct >= 80) {
      return tier === 'teacher' ? 'An outstanding achievement. You consistently demonstrate deep understanding of the syllabus.' : 'Excellent academic profile. Your commitment to your studies is highly commendable.';
    } else if (pct >= 70) {
      return tier === 'teacher' ? 'Very good performance. You show a strong grasp of most subject areas.' : 'A strong set of results. Keep up the consistent effort to reach the highest grades.';
    } else if (pct >= 60) {
      return tier === 'teacher' ? 'Good effort overall, but there is room for improvement in specific topics.' : 'Solid academic progress. Focus on refining your revision techniques for better outcomes.';
    } else if (pct >= 50) {
      return tier === 'teacher' ? 'Fair performance. More consistent revision and active class participation are required.' : 'Average results. You must dedicate more time to independent study to improve your grades.';
    } else {
      return tier === 'teacher' ? 'Below expected standard. Significant improvement is needed across multiple subjects.' : 'Results are concerning. A meeting with parents is necessary to discuss an academic recovery plan.';
    }
  } else if (curriculum === 'IB') {
     if (pct >= 85) {
      return tier === 'teacher' ? 'Exceptional inquiry and critical thinking skills demonstrated across all subjects.' : 'An outstanding profile reflecting the true traits of an IB learner.';
    } else if (pct >= 70) {
      return tier === 'teacher' ? 'Strong academic performance. Continue to develop your analytical and research skills.' : 'Very good progress. Your dedication to the rigorous IB curriculum is evident.';
    } else if (pct >= 50) {
      return tier === 'teacher' ? 'Satisfactory effort. Focus on deepening your conceptual understanding and application.' : 'Steady progress. More engagement with the core components will enhance your overall profile.';
    } else {
      return tier === 'teacher' ? 'Performance requires significant attention. Seek support to grasp the fundamental concepts.' : 'Academic intervention is required. Let us work together to build a stronger foundation.';
    }
  } else {
    // Default fallback
    if (pct >= 80) {
      return tier === 'teacher' ? 'Excellent performance. Keep up the high standard of work.' : 'Outstanding academic achievement.';
    } else if (pct >= 60) {
      return tier === 'teacher' ? 'Good work overall. Aim to push your boundaries further.' : 'Commendable effort and steady progress.';
    } else if (pct >= 40) {
      return tier === 'teacher' ? 'Fair result. More focus and dedication will yield better outcomes.' : 'Average performance. Additional effort is required.';
    } else {
      return tier === 'teacher' ? 'Below average. You must work harder and seek help where needed.' : 'Academic performance needs immediate improvement.';
    }
  }
}
