...
/**
 * Merge curriculum built-in assessment types with school-defined custom exams.
 * Custom exams are stored in KV as paav_custom_exams: [{ key, label, weight }]
 * Built-in assessments from curr.ASSESSMENT_TYPES always come first.
 * Duplicate keys are deduplicated (built-in wins).
 *
 * @param {Array} builtIn - curr.ASSESSMENT_TYPES array
 * @param {Array} custom  - paav_custom_exams array from DB
 * @returns {Array} merged, deduplicated assessment list
 */
export function mergeAssessments(builtIn = [], custom = []) {
  const seen = new Set((builtIn || []).map(a => a.key));
  const merged = [...(builtIn || [])];
  for (const c of (custom || [])) {
    if (c && c.key && !seen.has(c.key)) {
      seen.add(c.key);
      merged.push({ key: c.key, label: c.label || c.key, isCustom: true, weight: c.weight });
    }
  }
  return merged;
}
...
/**
 * Returns true if the grade corresponds to a senior/upper secondary level
 * for any curriculum (e.g. Grade 10-12 CBC, A-Level British, Grade 11-12 IB, etc.)
 */
export function isSeniorLevel(grade, curriculum = 'CBC') {
  const curr = getCurriculum(curriculum);
  // CBC — check SENIOR array
  if (curr.SENIOR && curr.SENIOR.includes(grade)) return true;
  // Fallback: check gradeGroup returns 'senior' or 'a-level'
  const grp = curr.gradeGroup ? curr.gradeGroup(grade) : '';
  return grp === 'senior' || grp === 'a-level';
}

/**
 * Returns the curricula-aware pathway options for senior level learners.
 */
export function getSeniorPathways(curriculum = 'CBC') {
  switch (curriculum) {
    case 'BRITISH':
      return ['Sciences', 'Humanities & Social Sciences', 'Arts', 'Combined / General'];
    case 'CAMBRIDGE':
      return ['Core Sciences', 'Extended Humanities', 'Arts & Technology', 'Combined'];
    case 'IB':
      return ['Sciences', 'Individuals & Societies', 'Arts', 'Language & Literature'];
    case 'MONTESSORI':
      return ['Academic', 'Vocational', 'Arts & Creative'];
    case 'TVET':
      return ['Artisan', 'Certificate', 'Diploma', 'Advanced Diploma'];
    case 'CBC':
    default:
      return ['STEM', 'Social Sciences', 'Arts & Sports', 'Technical Vocational Education & Training (TVET)', 'Languages', 'General'];
  }
}
 ...
 */
export function calcLearnerReportData(marks, adm, grade, term, subjects, cfg = null, curriculum = 'CBC', weights = null, mode = 'per-level', customExams = []) {
  const result = [];
  let totalAvgPts = 0;
  let totalEntered = 0;
  let totalAvgScore = 0;
  ...
  const curr = getCurriculum(curriculum);
  const assessments = mergeAssessments(curr.ASSESSMENT_TYPES || [{ key: 'mt1', label: 'Mid-Term' }], customExams);
 ...
 * @param {string}  curriculum - 'CBC' | 'BRITISH' | 'CAMBRIDGE' | 'IB' | 'MONTESSORI' | 'TVET'
 * @param {string}  tier       - 'teacher' | 'principal' | 'subject_teacher'
 * @param {string}  learnerId  - admission number (for deterministic variation)
 * @param {string}  [subject]  - optional subject name for subject-specific prefix
 * @param {string}  [pathway]  - optional senior-school pathway (e.g. 'STEM')
 */
export function getProfessionalRemarks(pct, curriculum = 'CBC', tier = 'teacher', learnerId = '', subject = '', pathway = '') {
  ...
  // Build a contextual prefix based on subject and pathway
  const subjPrefix = subject ? `In ${subject}, ` : '';
  const pathwayNote = pathway && pct >= 60
    ? ` Your commitment to the ${pathway} pathway is commendable.`
    : pathway && pct < 60
    ? ` Strengthening your ${pathway} pathway subjects will be key to your success.`
    : '';

  // Wrap the raw remark with context
  function wrap(remark) {
    return `${subjPrefix}${remark}${pathwayNote}`;
  }
  ...
  if (curriculum === 'CBC') {
    if (pct >= 80) {
      ...
      return wrap(tier === 'teacher' ? teacherRemarks[idx % teacherRemarks.length] : principalRemarks[idx % principalRemarks.length]);
    } else if (pct >= 60) {
      const teacherRemarks = [
    ...
    if (pct >= 80) {
      const t = ['Exceptional practical mastery! Your competency in this unit far exceeds the required standard.','Outstanding hands-on performance. Your technical skills are industry-ready.','Superb unit competency score. You consistently demonstrate professional-grade execution.'];
      const p = ['Outstanding CBET result. This learner is ready for workplace placement.','Exceptional trade competency. A credit to the vocational programme.','Top-tier technical performance. This learner sets the benchmark for peers.'];
      return wrap(tier === 'teacher' ? t[idx % t.length] : p[idx % p.length]);
    } else if (pct >= 60) {
      const t = ['Good competency level. Focus on the practical units where marks can still improve.','Satisfactory technical performance. Additional workshop practice will sharpen your skills.','Commendable effort. Consistent hands-on rehearsal will elevate your competency band.'];
      const p = ['Good occupational progress. More supervised practice will boost results.','Solid trade performance. Encourage additional time in the workshop.','Competent performance. Targeted reinforcement of weaker units is recommended.'];
      return wrap(tier === 'teacher' ? t[idx % t.length] : p[idx % p.length]);
    } else if (pct >= 40) {
      const t = ['Fair technical performance. Revisit the theoretical modules that underpin this practical unit.','Average competency. More disciplined practice sessions are needed before the next assessment.','Approaching the competency threshold. Seek guidance from your trade instructor.'];
      const p = ['Average occupational grade. A structured remedial workshop is recommended.','Fair CBET result. Increased supervised practice hours will help bridge the gap.','Competency not yet fully demonstrated. An additional assessment opportunity should be arranged.'];
      return wrap(tier === 'teacher' ? t[idx % t.length] : p[idx % p.length]);
    } else {
      const t = ['Below competency standard. Immediate remedial training and re-assessment are required.','Not yet competent in this unit. Please attend the catch-up workshop before re-sitting.','Significant improvement needed. Work closely with your trade instructor to address gaps.'];
      const p = ['Not yet competent. A formal remediation plan must be put in place.','Below standard. Parent/guardian and industry mentor consultation is advised.','Urgent intervention required to bring this learner to the minimum competency level.'];
      return wrap(tier === 'teacher' ? t[idx % t.length] : p[idx % p.length]);
    }
  } else if (curriculum === 'MONTESSORI') {
    if (pct >= 80) {
      const t = ['Exceptional self-directed learning! You demonstrate deep intrinsic motivation and mastery.','Outstanding inquiry skills. Your curiosity-driven approach to learning is exemplary.','Superb work-cycle engagement. You consistently reach mastery in self-chosen activities.','Brilliant independent research and hands-on exploration. Keep nurturing your natural love of learning.'];
      const p = ['An outstanding Montessori learner profile. Intrinsic motivation and deep focus are evident.','Exemplary self-directed achievement. This learner embodies the Montessori philosophy.','Outstanding mastery demonstrated across all learning areas. A truly self-motivated individual.'];
      return wrap(tier === 'teacher' ? t[idx % t.length] : p[idx % p.length]);
    } else if (pct >= 60) {
      const t = ['Good self-directed progress. Continue following your curiosity into deeper exploration.','Commendable work-cycle completion. Slightly more focused repetition will deepen mastery.','Solid engagement with the learning materials. Explore the extension activities to challenge yourself further.'];
      const p = ['Good independent progress. Guided extension activities are recommended.','Commendable Montessori performance. Encourage deeper exploration of interest areas.','Solid learning profile. The child shows good self-regulation and material engagement.'];
      return wrap(tier === 'teacher' ? t[idx % t.length] : p[idx % p.length]);
    } else if (pct >= 40) {
      const t = ['Developing self-direction. Encourage more time with the materials in a quiet, focused setting.','Average engagement with the prepared environment. Revisit foundational activities to build confidence.','Progressing steadily. Additional one-on-one guidance will help unlock deeper concentration.'];
      const p = ['Average self-directed performance. A review of the learning environment setup is recommended.','Developing mastery. Additional adult observation and gentle redirection may support progress.','Fair engagement. A parent-teacher conference to align home and school environments is advised.'];
      return wrap(tier === 'teacher' ? t[idx % t.length] : p[idx % p.length]);
    } else {
      const t = ['Below expected mastery level. Revisit the simpler preparatory materials to rebuild foundational skills.','Struggling with self-direction. Close observation and guided presentations are recommended.','Needs significant support. Daily one-on-one material presentations will help re-engage the learner.'];
      const p = ['Significant support required. An individual learning plan is recommended.','Below Montessori mastery threshold. Parent collaboration and specialist observation are advised.','Urgent intervention: the prepared environment may need to be reviewed to meet this child\'s developmental needs.'];
      return wrap(tier === 'teacher' ? t[idx % t.length] : p[idx % p.length]);
    }
  } else {
    // Default fallback
