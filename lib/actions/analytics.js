...
export async function getAcademicStats({ tenantId, grade, term, stream = '', curriculum = 'CBC' }) {
  console.log(`[Analytics] getAcademicStats: tenant=${tenantId}, grade=${grade}, term=${term}, stream=${stream}, curriculum=${curriculum}`);
  try {
    await ensureSchema();
    ...
    const finalGradeLearners = (gradeLearners || []).filter(l => !stream || l.stream === stream);

    const learnerMap = new Map();
    finalGradeLearners.forEach(l => learnerMap.set(l.adm, l));
    ...
    const emptyResult = {
      ...
      data: {
        ...
        studentCount: finalGradeLearners.length, enteredLearners: 0,
    ...
    const learnerAdms = new Set(finalGradeLearners.map(l => String(l.adm)));
    ...
    const expectedEntries      = Math.max(1, finalGradeLearners.length * expectedSubjectsCount);

    console.log(`[Analytics] Done. Entered: ${enteredLearners}/${finalGradeLearners.length}, curriculum: ${curriculum}`);
    ...
    return {
      ...
      data: JSON.parse(JSON.stringify({
        subjectMastery, genderComparison, streamComparison,
        assessmentComparison, levelDistribution, historicalProgression,
        studentCount:   finalGradeLearners.length,
