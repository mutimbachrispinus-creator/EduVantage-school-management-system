...
export function getClient() {
  ...
  try {
    ...
    if (normalizedUrl.startsWith('file:')) {
      // Use Node client for local file SQLite
      _client = createNodeClient({ url: normalizedUrl });
    } else {
      // Use Web client for remote Turso (Edge compatible)
      _client = createWebClient({ url: normalizedUrl, authToken: token });
    }
    return _client;
  } catch (e) {
  ...
  _schemaPromise = (async () => {
    try {
      ...
      const migrations = [
        ...
        "ALTER TABLE subscriptions ADD COLUMN billing_model TEXT DEFAULT 'flat'"
      ];
...
export async function kvUpdateLearner(oldAdm, details, tenantId = 'platform-master') {
  ...
  if (oldAdm !== newAdm) {
    ...
    console.error('[DB] kvUpdateLearner error:', e.message, { oldAdm, tenantId });
    throw e;
  }
}
    ...
    for (const l of rows) {
      stmts.push({
        sql: `INSERT INTO deleted_learners (
              ...
              ON CONFLICT(adm, tenant_id) DO UPDATE SET deleted_at = strftime('%s','now')`,
        args: [l.adm, tenantId, l.name, l.grade, l.sex, l.age, l.dob, l.stream, l.teacher, l.parent, l.phone, l.parentEmail, l.addr, l.t1, l.t2, l.t3, l.arrears, l.avatar, l.bloodGroup, l.allergies, l.medicalCondition, l.emergencyContact]
      });
    }
    stmts.push({ sql: 'DELETE FROM learners WHERE grade = ? AND tenant_id = ?', args: [grade, tenantId] });
    await batch(stmts);
    
    // Update KV cache
    const { value: learners } = await kvGetWithMeta('paav6_learners', tenantId);
    if (Array.isArray(learners)) {
      const updated = learners.filter(l => l.grade !== grade);
      await kvSet('paav6_learners', updated, tenantId);
    }
  }
}
