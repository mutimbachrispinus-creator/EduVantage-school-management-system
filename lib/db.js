...
/**
 * Completely remove a learner and all associated data.
 */
export async function kvDeleteLearner(adm, tenantId = 'platform-master') {
  ...
  // Update KV for paav_profiles (extended data)
  const { value: profiles } = await kvGetWithMeta('paav_profiles', tenantId);
  if (profiles && typeof profiles === 'object' && profiles[adm]) {
    const newProfiles = { ...profiles };
    delete newProfiles[adm];
    await kvSet('paav_profiles', newProfiles, tenantId);
  }
...
export async function recoverOrphanedData(tenantId = 'platform-master') {
  await ensureSchema();
  console.log('[DB] Starting data recovery for orphaned marks/payments...');
  
  // 1. Fetch all current learners to use as targets
  ...
  for (const orphan of orphanedAdms) {
    ...
    // We can't know the name easily, but we can look at where this ADM appears in other tables
    // and see if we can find a current learner who "took over" this history.
    // If the user just merged, the new ADM is in the 'learners' table.
    
    // HEURISTIC: If there is ONLY ONE learner in the same grade who was recently updated, 
    // or if the oldAdm is a partial match of a newAdm.
    
    // For now, let's look for any marks that were for "Grade 4" (if we can tell)
    // Actually, the most reliable way is to ask the user to just re-run the Merge with the NEW logic.
    // Wait! If they re-run the Merge now, my NEW logic will find the existing learner and the old orphaned data 
    // (if the old record was deleted, it won't find it).
  }
  
  // Update: I will add a logic to kvBulkAddLearners that also looks for ORPHANED data 
  // from the ADMs provided in the CSV and links them to the new ADMs.
  
  return orphanedAdms.length;
}
