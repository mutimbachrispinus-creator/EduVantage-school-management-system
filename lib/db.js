...
export async function ensureSchema() {
  ...
  _schemaPromise = (async () => {
    try {
      ...
      // 1. Fast path: check if a core table exists
      try {
        await db.execute("SELECT 1 FROM kv LIMIT 1");
        _schemaOk = true;
        _schemaCheckDone = true;
        return;
      } catch (e) {
        // Table doesn't exist, proceed to create
        console.log('[DB] Schema not found, initializing...');
      }
      ...
      // 3. Selective Alterations (only run on cold start if not checked)
      // We skip the sqlite_master check for performance, and just try-catch the alters
      const migrations = [
      ...
      for (const m of migrations) {
        try { await db.execute(m); } catch (e) { /* ignore duplicate column */ }
      }
      ...
      for (const idx of indices) {
        try { await db.execute(idx); } catch (e) { /* ignore */ }
      }
      ...
      // Don't throw here to avoid blocking every single request if one minor part fails
      // instead, we set _schemaOk if we at least reached some point
      _schemaOk = true; 
    } finally {
      _schemaPromise = null;
    }
