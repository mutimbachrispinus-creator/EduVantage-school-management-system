...
export async function ensureSchema() {
  ...
  try {
    ...
    const tblCols = {};
    try {
      const pragmaStmts = tables.map(tbl => ({ sql: `PRAGMA table_info(${tbl})`, args: [] }));
      const pragmaResults = await _batch(pragmaStmts);
      
      for (let i = 0; i < tables.length; i++) {
        const tbl = tables[i];
        // libsql/client batch returns an array of ResultSet objects
        const info = pragmaResults[i].rows;
        tblCols[tbl] = new Set(info.map(c => c.name));
      }
    } catch (e) {
      for (const tbl of tables) tblCols[tbl] = new Set();
    }

    const indexStmts = [];
    for (const m of migrationStmts) {
      if (m.t === 'index') {
        indexStmts.push({ sql: m.sql, args: [] });
      }
    }
    if (indexStmts.length > 0) {
      try { await _batch(indexStmts); } catch (e) {}
    }

    for (const m of migrationStmts) {
      if (m.t !== 'index' && tblCols[m.t] && !tblCols[m.t].has(m.c)) {
        try {
