...
export function getClient() {
  ...
  if (!url) {
    console.error('[DB] Missing TURSO_URL. Environment variables:', { 
    ...
    try {
      ...
      const stmts = [
        ...
        `CREATE TABLE IF NOT EXISTS subscriptions (
          ...
          expires_at TEXT,
          learner_limit INTEGER DEFAULT 50,
          updated_at INTEGER DEFAULT (strftime('%s','now'))
        `,
      ...
      try {
        ...
        // Migration: Add learner_limit if missing
        try {
          await db.execute('ALTER TABLE subscriptions ADD COLUMN learner_limit INTEGER DEFAULT 50');
...
export async function kvDeleteLearner(adm, tenantId = 'platform-master') {
  ...
  const stmts = [
    ...
    { sql: 'DELETE FROM marks WHERE adm = ? AND tenant_id = ?', args: [adm, tenantId] },
    { sql: 'DELETE FROM paylog WHERE adm = ? AND tenant_id = ?', args: [adm, tenantId] },
    { sql: 'DELETE FROM attendance WHERE grade_date_adm LIKE ? AND tenant_id = ?', args: [`%|${adm}`, tenantId] }
  ];
  await batch(stmts);
