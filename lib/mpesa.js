 ...
 */
export async function getAccessToken(customKey = null, customSecret = null) {
  const key    = customKey    || process.env.MPESA_CONSUMER_KEY    || '';
  const secret = customSecret || process.env.MPESA_CONSUMER_SECRET || '';
  ...
  const res = await fetchWithRetry(() => fetch(`${baseURL()}/oauth/v1/generate?grant_type=client_credentials`, {
    headers: { Authorization: `Basic ${credentials}` },
  }));

  if (!res.ok) {
    logDarajaEvent('oauth_failed', { status: res.status });
 ...
 */
export async function stkPush({ phone, amount, accountRef, description, shortcode: sCode, passkey: pKey, consumerKey, consumerSecret, env }) {
  try {
    const token     = await getAccessToken(consumerKey, consumerSecret);
    const shortcode = sCode || process.env.MPESA_SHORTCODE || '';
    const passkey   = pKey  || process.env.MPESA_PASSKEY   || '';
