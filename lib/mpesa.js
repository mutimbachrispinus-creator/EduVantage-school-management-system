...
export async function stkPush({ phone, amount, accountRef, description, shortcode: sCode, passkey: pKey, consumerKey, consumerSecret, env, callbackUrl }) {
   try {
     ...
     const shortcode = String(sCode || process.env.MPESA_SHORTCODE || process.env.DARAJA_SHORTCODE || '').trim();
     let passkey     = String(pKey  || process.env.MPESA_PASSKEY   || process.env.DARAJA_PASSKEY   || '').trim();
     const callback  = String(callbackUrl || process.env.MPESA_CALLBACK_URL || process.env.DARAJA_CALLBACK_URL || '').trim();

     // Centralized override: ensure sandbox requests for 174379 always use Safaricom's default sandbox passkey
     ...
     const activeEnv = env || process.env.MPESA_ENV || 'sandbox';
     const darajaEnv = ['live', 'production', 'prod'].includes(String(activeEnv).toLowerCase().trim()) ? 'production' : 'sandbox';
