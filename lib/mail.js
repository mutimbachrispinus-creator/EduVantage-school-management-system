...
// Global client — used when no per-tenant key is provided
 ...
 * @param {string}          [opts.resendApiKey]  — per-tenant key from paav_integration_keys
 * @param {string}          [opts.fromEmail]     — per-tenant sender from paav_integration_keys
 */
export async function sendEmail({ to, subject, html, text, resendApiKey, fromEmail }) {
  // Resolve client: per-tenant key takes priority, fall back to env-var global client
  const client = resendApiKey ? new Resend(resendApiKey) : resend;

  if (!client) {
    console.error('[Mail] Resend API key not configured (env or integration keys). Email not sent.');
    return { error: 'Resend API key not configured' };
  }

  const from = fromEmail
    ? `School Portal <${fromEmail}>`
    : 'EduVantage Portal <portal@eduvantage.app>';
 ...
 */
export function getReportCardTemplate({ learnerName, term, exam, year, totalPts, maxPts, pct, generalLevel, promoStatus, link }) {
  return `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
      <h2 style="color: #1e293b; text-align: center;">Termly Progress Report</h2>
      <p>Dear Parent,</p>
      <p>The academic report for <strong>${learnerName}</strong> for <strong>Term ${term} ${year}</strong> (Exam: ${exam}) is now available.</p>
      ...
      <div style="background: #f8fafc; padding: 15px; border-radius: 8px; margin: 20px 0; text-align: center;">
        ...
        <div style="margin-top: 10px; font-size: 14px; color: #475569;">General Level: <strong>${generalLevel}</strong></div>
      </div>
      ...
      <hr style="border: 0; border-top: 1px solid #eee; margin: 30px 0;" />
      <p style="font-size: 12px; color: #94a3b8; text-align: center;">
        School Portal Management System<br />
  ...
  return `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
      <div style="text-align: center; margin-bottom: 20px;">
        <img src="https://eduvantage.app/eduvantage-logo.png" alt="EduVantage Logo" style="width: 80px; height: 80px;" />
        <h2 style="color: #1E40AF; margin-top: 10px;">Fee Statement Reminder</h2>
      </div>
      ...
      <div style="text-align: center; margin: 30px 0;">
        <a href="https://portal.eduvantage.app" style="background: #1E40AF; color: #fff; padding: 12px 25px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block;">
          Go to Portal
        </a>
      </div>
      ...
      <p style="font-size: 11px; color: #94a3b8; text-align: center;">
        School Portal · Automated Reminder<br />
