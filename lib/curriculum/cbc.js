...
export function getScale(grade, cfg = null, subject = null) {
  ...
  // 2. Check for subject + level specific scale
  if (subject && cfg?.subjects?.[subject]?.[levelKey]) {
    return cfg.subjects[subject][levelKey];
  }

  // 3. Fallback to subject-wide scale (old format)
  if (subject && cfg?.subjects?.[subject] && Array.isArray(cfg.subjects[subject])) {
    return cfg.subjects[subject];
  }

  // 4. Fallback to uniform if set
  if (cfg?.uniform) return cfg.uniform;

  // 5. Fallback to per-level
