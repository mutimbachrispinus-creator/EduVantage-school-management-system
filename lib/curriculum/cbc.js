...
export const DEFAULT_WEIGHTS = { op1: 0.1, mt1: 0.3, et1: 0.6 };
...
export const CATEGORIES = [
  { title: 'Pre-School', grades: PRE, color: '#8B5CF6', levelKey: 'pre' },
  ...
  { title: 'Junior Secondary', grades: JSS, color: '#F59E0B', levelKey: 'junior' },
  { title: 'Senior School', grades: SENIOR, color: '#EF4444', levelKey: 'senior' },
];
...
export function gradeGroup(grade) {
  ...
  if (JSS.includes(grade)) return 'jss';
  if (SENIOR.includes(grade)) return 'senior';
  return 'primary13'; 
}

export const JSS_SCALE = [
  { min: 90, lv: 'EE1', pts: 8, c: '#065F46', bg: '#D1FAE5', desc: 'Exceeding Expectation — Exceptional' },
  { min: 75, lv: 'EE2', pts: 7, c: '#059669', bg: '#A7F3D0', desc: 'Exceeding Expectation — Very Good' },
  { min: 58, lv: 'ME1', pts: 6, c: '#1D4ED8', bg: '#BFDBFE', desc: 'Meeting Expectation — Good' },
  { min: 41, lv: 'ME2', pts: 5, c: '#2563EB', bg: '#DBEAFE', desc: 'Meeting Expectation — Fair' },
  { min: 31, lv: 'AE1', pts: 4, c: '#B45309', bg: '#FDE68A', desc: 'Approaching Expectation — Needs Improvement' },
  { min: 21, lv: 'AE2', pts: 3, c: '#92400E', bg: '#FEF3C7', desc: 'Approaching Expectation — Below Average' },
  { min: 11, lv: 'BE1', pts: 2, c: '#DC2626', bg: '#FEE2E2', desc: 'Below Expectation — Well Below Average' },
  { min:  0, lv: 'BE2', pts: 1, c: '#991B1B', bg: '#FCA5A5', desc: 'Below Expectation — Minimal' },
];
...
export const GRADING_CONFIG = [
  { key: 'k6', title: '🟢 Pre-School & Primary (K-6)', scale: PRIMARY_SCALE },
  { key: 'junior', title: '🟡 Junior School (7–9)', scale: JSS_SCALE },
  { key: 'senior', title: '🔴 Senior School (10–12)', scale: JSS_SCALE },
];

export function shouldRankByMarks(grade) {
  return [...PRE, ...LOWER, ...UPPER].includes(grade);
}
...
export function getScale(grade, cfg = null, subject = null, mode = 'per-level') {
  // 1. Determine levelKey
  let levelKey = 'k6';
  if (SENIOR.includes(grade)) levelKey = 'senior';
  else if (JSS.includes(grade)) levelKey = 'junior';

  // 2. Per-Subject Mode
  if (mode === 'per-subject' && subject && cfg?.subjects?.[subject]) {
    const sCfg = cfg.subjects[subject];
    if (sCfg[levelKey]) return sCfg[levelKey];
    if (Array.isArray(sCfg)) return sCfg; // Fallback to old format
  }

  // 3. Uniform Mode
  if (mode === 'uniform' && cfg?.uniform) return cfg.uniform;

  // 4. Per-Level Mode (Default)
  if (levelKey === 'senior') return cfg?.senior || JSS_SCALE;
  if (levelKey === 'junior') return cfg?.junior || JSS_SCALE;
  return cfg?.k6 || PRIMARY_SCALE;
}
