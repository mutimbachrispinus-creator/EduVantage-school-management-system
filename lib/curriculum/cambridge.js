...
export const ASSESSMENT_TYPES = [
  { key: 'op1', label: '📝 Diagnostic' },
  { key: 'mt1', label: '📖 Mid-Term' },
  { key: 'et1', label: '📋 Final Exam' },
];
...
export function getScale(grade, cfg = null, subject = null) {
  let levelKey = 'stage';
  if (ALEVEL.includes(grade)) levelKey = 'alevel';
  else if (IGCSE.includes(grade)) levelKey = 'igcse';
  ...
  if (levelKey === 'alevel') return cfg?.alevel || ALEVEL_SCALE;
  if (levelKey === 'igcse')  return cfg?.igcse  || IGCSE_SCALE;
