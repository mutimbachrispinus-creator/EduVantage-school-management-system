...
export function gradeGroup(grade) {
  if (['KINDERGARTEN','RECEPTION'].includes(grade)) return 'early_years';
  if (PRIMARY.includes(grade))    return 'cambridge_primary';
  if (SECONDARY1.includes(grade)) return 'cambridge_lower_secondary';
  if (IGCSE.includes(grade))      return 'cambridge_igcse';
  return 'cambridge_alevel';
}
...
// Cambridge Primary/Lower Secondary Scale (Gold, Silver, Bronze)
