 ...
 * @param {object} [creds]
 */
export async function sendFeeReminderSMS({ parentPhone, learnerName, balance, paybill, admNo, schoolName }, creds = {}) {
  const prefix = schoolName ? `[${schoolName}]\n` : '';
  const message =
    `${prefix}School Fee Reminder\n` +
    `Student: ${learnerName}\n` +
 ...
 */
export function getResultNotificationMessage(learnerName, term, totalPts, maxPts, schoolName) {
  const prefix = schoolName ? `[${schoolName}]\n` : '';
  return `${prefix}School Results Notice\n` +
